    ...
    if res['ret'] == 0:  # 0代表请求成功
        ...
        for obj in res['data']['face_list']:
            ...
            if len(res['data']['face_list']) > 1:  # 检测到多个人脸，就把信息写入人脸框内
                font = ImageFont.truetype('D:/shiyan/itchat/wechat_face/yahei.ttf', w // 8, encoding='utf-8')  # 提前把字体文件下载好
                draw.text((x + 10, y + 10), '性别 :' + gender, (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 1), '年龄 :' + str(obj['age']), (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 2), '表情 :' + expression, (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 3), '魅力 :' + str(obj['beauty']), (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 4), '眼镜 :' + glass, (105,22,57), font=font)
            elif img_width - x - w < 170:  # 避免图片太窄，导致文字显示不完全
                font = ImageFont.truetype('D:/shiyan/itchat/wechat_face/yahei.ttf', w // 8, encoding='utf-8')
                draw.text((x + 10, y + 10), '性别 :' + gender, (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 1), '年龄 :' + str(obj['age']), (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 2), '表情 :' + expression, (105,22,57), font=font)
                draw.text((x + 10, y + 10 + delt * 3), '魅力 :' + str(obj['beauty']), (105,22,57), font=font)
                ...
                font = ImageFont.truetype('D:/shiyan/itchat/wechat_face/yahei.ttf', 20, encoding='utf-8')
                draw.text((x + w + 10, y + 10), '性别 :' + gender, (105,22,57), font=font)
                draw.text((x + w + 10, y + 10 + delt * 1), '年龄 :' + str(obj['age']), (105,22,57), font=font)
                draw.text((x + w + 10, y + 10 + delt * 2), '表情 :' + expression, (105,22,57), font=font)
                draw.text((x + w + 10, y + 10 + delt * 3), '魅力 :' + str(obj['beauty']), (105,22,57), font=font)
                draw.text((x + w + 10, y + 10 + delt * 4), '眼镜 :' + glass, (105,22,57), font=font)

            draw.rectangle((x, y, x + w, y + h), outline="#F8413E")  # 画出人脸方框
            cv2img = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)  # 把 pil 格式转换为 cv
            cv2.imwrite(img, cv2img)  # 保存图片到 face 文件夹下
        return 'success'
    else:
        return 'fail'
